...
_REPO_ROOT = _THIS_DIR.parent                             # root of the repo (where ui_all.html is located)
_STATIC_CANDIDATES = [
    _THIS_DIR / "static",
    _REPO_ROOT / "static",
]
for _static_dir in _STATIC_CANDIDATES:
    if _static_dir.exists() and _static_dir.is_dir():
        ...
        break  # assemble the first one that exists

# --- UI: serve ui_all.html in "/" ---
# First search in the root directory (as in your structure), then in email_server/templates
_UI_CANDIDATES = [
...
async def serve_ui(request: Request):
    ...
    ui_llm_server   = os.getenv("UI_LLM_SERVER", "http://127.0.0.1:8001")  # example
